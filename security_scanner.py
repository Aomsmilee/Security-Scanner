...
class SentinelApp(ctk.CTk):
    ...
    def build_ui(self):
        ...
        # ===== Tab =====
        self.tab_var = tk.StringVar(value="url")
    ...
    def switch_tab(self, tab):
        ...
        for v in ["url", "file", "verify", "history"]:
        ...
        {
            "file": self.show_file_tab,
            "url": self.show_url_tab,
            "verify": self.show_verify_tab,
    ...
    def show_file_tab(self):
        ...
        ctk.CTkButton(btn_row, text="📂  Select file", width=140, height=38,
                      ...
                      hover_color="#BFDBFE",
                      command=self.browse_file).pack(side="left", padx=(0, 12))
        ctk.CTkButton(btn_row, text="🔍  Scan", width=120, height=38,
                      corner_radius=8, font=ctk.CTkFont(size=13),
                      fg_color=COLOR_PRIMARY, text_color="white",
                      hover_color="#1D4ED8",
                      command=self.check_file).pack(side="left")

        self.file_result = self.result_box(card)
        self.selected_file = None

    def browse_file(self):
        path = filedialog.askopenfilename() # <--- จุดที่ส่ง Request หา OS
        if path:
    ...
    def check_file(self):
        if not self.selected_file:
            self.show_result(self.file_result, "⚠️ Please select a file", "warning")
            return
        self.show_result(self.file_result, "⏳ Scanning...", "info")
    ...
    def scan_file(self):
        try:
            ...
            self.add_history("File", name, result)
            self.after(0, lambda: self.show_result(self.file_result, result))
        except Exception as e:
            self.after(0, lambda: self.show_result(self.file_result, f"❌ Something went wrong: {e}", "danger"))
    ...
    def show_url_tab(self):
        ...
        ctk.CTkButton(card, text="🔍  Scan Link", height=40,
                      ...
                      fg_color=COLOR_PRIMARY, text_color="white",
                      hover_color="#1D4ED8",
                      command=self.check_url).pack(anchor="w")

        self.url_result = self.result_box(card)

    def check_url(self):
        url = self.url_var.get().strip()
        if not url:
            self.show_result(self.url_result, "⚠️ Please insert the link", "warning")
            return
        self.show_result(self.url_result, "⏳ Scanning...", "info")
    ...
    def scan_url(self, url):
        try:
            ...
            self.add_history("URL", url, result)
            self.after(0, lambda: self.show_result(self.url_result, result))
        except Exception as e:
            self.after(0, lambda: self.show_result(self.url_result, f"❌ Something went wrong: {e}", "danger"))

    # ========== TAB: VERIFY HASH ==========
    def show_verify_tab(self):
        card = self.card(self.content)

        ctk.CTkLabel(card, text="Verify Hash",
                     font=ctk.CTkFont(size=15, weight="bold"), text_color=COLOR_TEXT).pack(anchor="w", pady=(0, 4))
        ...
        ctk.CTkButton(btn_row, text="📂  Select file", width=140, height=38,
                      ...
                      fg_color=COLOR_ACCENT, text_color=COLOR_PRIMARY,
                      hover_color="#BFDBFE",
                      command=self.browse_verify_file).pack(side="left", padx=(0, 12))

        ctk.CTkButton(btn_row, text="🔐  Check", width=130, height=38,
                      corner_radius=8, font=ctk.CTkFont(size=13),
                      fg_color=COLOR_PRIMARY, text_color="white",
                      hover_color="#1D4ED8",
                      command=self.check_hash_file).pack(side="left")
        ...
        self.verify_result = self.result_box(card)
    ...
    def check_hash_file(self):
        ...
        if not original_hash:
            self.show_result(self.verify_result, "⚠️ Please include the original hash", "warning")
            return
        if not self.verify_selected_file:
            self.show_result(self.verify_result, "⚠️ Please select a file", "warning")
            return
        self.show_result(self.verify_result, "⏳ Scanning...", "info")
        threading.Thread(target=self.run_verify, args=(original_hash,), daemon=True).start()

    def run_verify(self, original_hash):
        try:
            ...
            if match:
                ...
                self.add_history("Verify Hash", name, result)
            else:
            ...
            self.after(0, lambda: self.show_result(self.verify_result, result))
        except Exception as e:
            self.after(0, lambda: self.show_result(self.verify_result, f"❌ Something went wrong: {e}", "danger"))
    ...
    def show_result(self, box, text, kind=None):
        if kind is None:
    ...
    def add_history(self, scan_type, name, result):
        self.history.append({
            "type": scan_type,
            "name": name,
            "result": result,
